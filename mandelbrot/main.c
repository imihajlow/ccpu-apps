...
static bool diverged(fp32 c_re, fp32 c_im) {
    return (c_re > (2l << 28)) || (c_re < (-2l << 28)) || (c_im > (2l << 28)) || (c_im < (-2l << 28));
}

static uint8_t iterate(fp32 c_re, fp32 c_im) {
    uint8_t step = 0;
    fp32 z_re = c_re;
    fp32 z_im = c_im;
    while (step < MAX_STEPS && !diverged(z_re, z_im)) {
        // z = z^2 + c
        fp32 z_re_2 = fp32_28_mul(z_re, z_re);
        fp32 z_im_2 = fp32_28_mul(z_im, z_im);
        fp32 z_re_im = fp32_28_mul(z_re, z_im);
...
static fp32 julia_c_im;
static fp32 julia_c_re;

static uint8_t iterate_julia(fp32 z0_re, fp32 z0_im) {
    uint8_t step = 0;
    fp32 z_re = z0_re;
    fp32 z_im = z0_im;
    while (step < MAX_STEPS && !diverged(z_re, z_im)) {
...
static void plot(fp32 x0, fp32 y0, fp32 step, bool is_julia) {
    char *p_color = VGA_COLOR_SEG;
    fp32 double_step = step + step;
    fp32 im_top = y0;
    fp32 im_bot = y0 - step;
...
apply_zoom: ;
        fp32 center_x = x0 + cursor_x * step;
        fp32 center_y = y0 - cursor_y * step * 2;
        step = new_step;
